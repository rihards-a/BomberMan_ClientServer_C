...
/*  set to 2 if wish to use '.' symbols in between chars for padding 
    currently stopped developping non padded version, use 2 only.   */
#define DOT_PADDING 2 
/*  max amount of symbols representing a single element on the map 
...
#define SIDE_BAR_WIDTH MAX_PLAYER_NAME_LENGTH + 4 + 1 + 4 + 10
...
uint8_t BLOCK_SIZE; /* size of each cell in characters */
...
int main() {
    ...
    noecho();       /* don't echo userinput back */
    ...
    while (1) {
        ...
        if (resized) {
            ...
            endwin();
            refresh();
            // clear();                             // doesn't seem necessary
            // wresize(TERMINAL_WIN, LINES, COLS);  // doesn't seem necessary
            werase(TERMINAL_WIN);

            /* calculate maximum possible BLOCK_SIZE */
            while (BLOCK_SIZE > 1) {
                /* extra padding for pleasnt warping */
        ...
        usleep(200000); // 20 ticks per second
        draw_game_board();
    }
    return 0;
}
...
void draw_game_board() {
    ...
    } else { /* draw borders around objects */
        for (int i = 0; i < GAME_MAP.height; i++) {
            for (int j = 0; j < GAME_MAP.width; j++) {
                char cell = GAME_MAP.data[i * GAME_MAP.width + j];
                if (cell == '.') { continue; } /* skip empty cells */
